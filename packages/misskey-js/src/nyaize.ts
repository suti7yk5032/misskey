...
export function nyaize(text: string): string {
	return text
		...
		// en-US
		.replace(enRegex1, (match, suffix) => `${match[0]}${suffix === 'A' ? 'YA' : 'ya'}`)
		.replace(enRegex2, (_match, prefix, suffix) => `${prefix}${suffix === 'ING' ? 'YAN' : 'yan'}`)
		.replace(enRegex3, (_match, prefix, suffix) => `${prefix}${suffix === 'ONE' ? 'NYAN' : 'nyan'}`)
		// ko-KR
